  ...
  class PersonalDetails
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PersonalDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PersonalDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@tax_id.nil? && @tax_id.to_s.length > 255
        invalid_properties.push('invalid value for "tax_id", the character length must be smaller than or equal to 255.')
      end
    ...
    def valid?
      ...
      return false if !@tax_id.nil? && @tax_id.to_s.length > 255
      true
    end
    ...
    def tax_id=(tax_id)
      ...
      if tax_id.to_s.length > 255
        fail ArgumentError, 'invalid value for "tax_id", the character length must be smaller than or equal to 255.'
      end
    ...
    def hash
      [first_name, last_name, email, phone, birth_date, tax_id, address].hash
    end
