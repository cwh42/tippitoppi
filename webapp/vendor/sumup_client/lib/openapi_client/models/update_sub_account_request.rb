  ...
  class UpdateSubAccountRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UpdateSubAccountRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UpdateSubAccountRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@username.nil? && @username.to_s.length > 256
        invalid_properties.push('invalid value for "username", the character length must be smaller than or equal to 256.')
      end
    ...
    def valid?
      ...
      return false if !@username.nil? && @username.to_s.length > 256
      true
    end
    ...
    def username=(username)
      ...
      if username.to_s.length > 256
        fail ArgumentError, 'invalid value for "username", the character length must be smaller than or equal to 256.'
      end
    ...
    def hash
      [password, username, disabled, nickname, permissions].hash
    end
