  ...
  class Operator
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Operator` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Operator`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      account_type_validator = EnumAttributeValidator.new('String', ["operator", "normal"])
    ...
    def account_type=(account_type)
      validator = EnumAttributeValidator.new('String', ["operator", "normal"])
    ...
    def hash
      [id, username, nickname, disabled, created_at, updated_at, permissions, account_type].hash
    end
