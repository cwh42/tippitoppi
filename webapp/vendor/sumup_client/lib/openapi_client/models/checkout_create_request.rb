  ...
  class CheckoutCreateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CheckoutCreateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CheckoutCreateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @checkout_reference.nil?
        invalid_properties.push('invalid value for "checkout_reference", checkout_reference cannot be nil.')
      end
      ...
      if @merchant_code.nil?
        invalid_properties.push('invalid value for "merchant_code", merchant_code cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @checkout_reference.nil?
      return false if @checkout_reference.to_s.length > 90
      return false if @amount.nil?
      return false if @currency.nil?
      return false if @merchant_code.nil?
      purpose_validator = EnumAttributeValidator.new('String', ["CHECKOUT", "SETUP_RECURRING_PAYMENT"])
    ...
    def purpose=(purpose)
      validator = EnumAttributeValidator.new('String', ["CHECKOUT", "SETUP_RECURRING_PAYMENT"])
    ...
    def hash
      [checkout_reference, amount, currency, merchant_code, pay_to_email, description, return_url, customer_id, purpose, id, status, date, valid_until, transactions, redirect_url].hash
    end
