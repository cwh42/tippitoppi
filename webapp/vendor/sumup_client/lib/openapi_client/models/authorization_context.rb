...
module OpenapiClient
  # Ext authz agent authorization context
  class AuthorizationContext
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::AuthorizationContext` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::AuthorizationContext`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'scp')
        if (value = attributes[:'scp']).is_a?(Array)
          self.scp = value
        end
      else
        self.scp = nil
    ...
    def list_invalid_properties
      ...
      if @scp.nil?
        invalid_properties.push('invalid value for "scp", scp cannot be nil.')
      end
    ...
    def hash
      [client_id, scp, ext, sub, aud].hash
    end
