  ...
  class BankAccountPayload
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::BankAccountPayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::BankAccountPayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      account_type_validator = EnumAttributeValidator.new('String', ["CURRENT", "SAVINGS"])
      return false unless account_type_validator.valid?(@account_type)
      return false if @account_holder_name.nil?
      status_validator = EnumAttributeValidator.new('String', ["OPEN"])
      return false unless status_validator.valid?(@status)
      account_category_validator = EnumAttributeValidator.new('String', ["PERSONAL", "BUSINESS"])
    ...
    def account_type=(account_type)
      validator = EnumAttributeValidator.new('String', ["CURRENT", "SAVINGS"])
    ...
    def hash
      [bank_code, branch_code, account_number, iban, swift, account_type, account_holder_name, check_digit, primary, status, account_category].hash
    end
