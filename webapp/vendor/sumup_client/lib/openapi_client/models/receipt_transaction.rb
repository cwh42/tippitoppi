  ...
  class ReceiptTransaction
    ...
    def self.openapi_types
      {
        ...
        :'products' => :'Array<ReceiptTransactionProductsInner>',
        :'vat_rates' => :'Array<ReceiptTransactionVatRatesInner>',
        :'events' => :'Array<ReceiptEvent>',
        :'receipt_no' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ReceiptTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ReceiptTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'receipt_no')
        self.receipt_no = attributes[:'receipt_no']
      end
    end
    ...
    def hash
      [transaction_code, amount, vat_amount, tip_amount, currency, timestamp, status, payment_type, entry_mode, verification_method, card, installments_count, products, vat_rates, events, receipt_no].hash
    end
