...
module OpenapiClient
  # Details on the acceptance of the SumUp Terms of Service Agreement
  class ToSAcceptance
    # The Unix timestamp marking when the user accepted Terms of Service agreement.
    attr_accessor :accepted_at

    # The IP address from which the user accepted Terms of Service agreement.
    attr_accessor :ip_address

    # The user agent of the browser from which the user accepted Terms of Service agreement.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ToSAcceptance` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ToSAcceptance`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [accepted_at, ip_address, user_agent].hash
    end
