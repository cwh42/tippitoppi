...
module OpenapiClient
  class GetPaymentMethods200Response
    ...
    def self.openapi_types
      {
        :'available_payment_methods' => :'Array<GetPaymentMethods200ResponseAvailablePaymentMethodsInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetPaymentMethods200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetPaymentMethods200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'available_payment_methods')
        if (value = attributes[:'available_payment_methods']).is_a?(Array)
          self.available_payment_methods = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          available_payment_methods == o.available_payment_methods
    end
    ...
    def hash
      [available_payment_methods].hash
    end
