...
module OpenapiClient
  # Ext authz agent signature context
  class SignatureContext
    attr_accessor :body_digest

    # Application-specific data provided by the signer which should be used when authorizing request together with `body_digest`.
    attr_accessor :meta

    # Method used to authenticate the requester when obtaining the signature.
    attr_accessor :amr
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SignatureContext` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SignatureContext`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'body_digest')
        self.body_digest = attributes[:'body_digest']
      end
