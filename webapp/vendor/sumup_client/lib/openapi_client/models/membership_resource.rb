  ...
  class MembershipResource
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::MembershipResource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::MembershipResource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'type')
        if (value = attributes[:'type']).is_a?(Hash)
          self.type = value
        end
      else
        self.type = nil
    ...
    def list_invalid_properties
      ...
      if !@logo.nil? && @logo.to_s.length > 256
        invalid_properties.push('invalid value for "logo", the character length must be smaller than or equal to 256.')
      end
    ...
    def valid?
      ...
      return false if !@logo.nil? && @logo.to_s.length > 256
    ...
    def logo=(logo)
      ...
      if logo.to_s.length > 256
        fail ArgumentError, 'invalid value for "logo", the character length must be smaller than or equal to 256.'
      end
    ...
    def hash
      [id, type, name, logo, created_at, updated_at, attributes].hash
    end
