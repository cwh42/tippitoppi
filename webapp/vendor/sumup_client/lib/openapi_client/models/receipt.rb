  ...
  class Receipt
    ...
    def self.openapi_types
      {
        ...
        :'emv_data' => :'Object',
        :'acquirer_data' => :'ReceiptAcquirerData'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Receipt` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Receipt`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'transaction_data')
        self.transaction_data = attributes[:'transaction_data']
      end
      ...
      if attributes.key?(:'emv_data')
        self.emv_data = attributes[:'emv_data']
      end

      if attributes.key?(:'acquirer_data')
        self.acquirer_data = attributes[:'acquirer_data']
      end
    end
    ...
    def hash
      [transaction_data, merchant_data, emv_data, acquirer_data].hash
    end
